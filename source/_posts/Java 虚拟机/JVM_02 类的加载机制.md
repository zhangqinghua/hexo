...
#### 链接
链接阶段可以分成 3 个步骤：**验证**、**准备**、**解析**。

**验证**阶段用于确保被加载的类的正确性。

**验证**是链接阶段的第一步，这一阶段的目的是为了确保 `Class` 文件的字节流中包含的信息符合当前虚拟机的要求，并且不会危害虚拟机自身的安全。验证阶段大致会完成 4 个阶段的检验动作：
...
1. 初始化某个类的子类，则其父类也会被初始化。
1. 访问某个类或接口的静态变量，或者对该静态变量赋值。
1. 调用类的静态方法。
1. 反射，如 `Class.forName("com.shengsiyuan.Test")`。
...
1. `ClassLoader.loadClass()` 只干一件事情，就是将 .class 文件加载到 JVM 中，不会执行 `static` 中的内容,只有在 `newInstance` 才会去执行 `static` 块。

> 注：`Class.forName(name, initialize, loader)` 带参函数也可控制是否加载 `static` 块。并且只有调用了 `newInstance()` 方法采用调用构造函数，创建类的对象。
...
1. 一个类加载到虚拟机中一共有几个步骤，这些步骤的顺序哪些是固定的，哪些是不固定的，为什么不固定？
